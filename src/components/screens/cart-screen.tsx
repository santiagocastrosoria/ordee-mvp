...
const qtyBtn =
  "inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-brand-border bg-brand-card text-base font-medium text-brand-ink shadow-sm ordee-tap-sm hover:bg-brand-soft hover:border-brand-muted/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-ink/20 disabled:pointer-events-none disabled:opacity-40";
...
export function CartScreen({ restaurantSlug, basePath }: CartScreenProps) {
  ...
  const changeQty = (id: string, next: number) => {
    setItems(updateItemQuantity(restaurantSlug, id, next));
  };

  const removeLine = (id: string) => {
    setItems(removeFromCart(restaurantSlug, id));
  };
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-1 sm:gap-1.5">
                  <button
                    ...
                    className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-brand-border bg-brand-card text-brand-muted shadow-sm ordee-tap-sm hover:border-red-200 hover:bg-red-50 hover:text-red-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-300/50"
